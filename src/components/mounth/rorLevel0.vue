...
export default{
  ...
  created(){
    this.Mdetail();
    this.Titlename();//根据进入人员级别动态渲染title
    jzmove();
  },
  computed:mapGetters([
      'roleLevelName',
  ...
  methods:{
   ...mapActions([
      ...
      watchElemt(){
          this.types = this.getEventTrigger(event).getElementsByTagName("div")[1].getElementsByTagName("div")[0].innerHTML;
            this.$store.dispatch('types',this.types);
      },
      ...
      Titlename(){
        if(this.role == '普通科员' || this.role == "普通副科长"){
          this.$store.commit('roleLevelNameFC',1);//根据进入人员级别动态渲染title
        }else{
          this.$store.commit('roleLevelNameFC',3);
        }
      },
      roleverPD(){
